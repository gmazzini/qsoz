...
int qsoz_proc_main(void){
  ...
  if(act==21){ // cbr out button
    ...
    fclose(fp);
    printf("<pre>");
    printf("<pre><a href='/files/%s' download>Download Cabrillo</a>\n",aux1);
    if(adif_rec.value[2][0]=='\0')printf("from:%s to:%s\n",adif_rec.value[0],adif_rec.value[1]);
    else printf("contest:%s\n",adif_rec.value[2]);
    printf("</pre>");
    goto end;
  ...
  if(act==31){ // contest details button
    ...
    printf("\n");
    printf("</pre>");
    if(!qsoz_contest_details_print(con,esc_mycall,esc_contest))printf("<pre>Contest details unavailable</pre>");
    goto end;
  }
